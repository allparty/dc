class Process:
    def __init__(self, pid):
        ...
        self.waiting_for = None  # List of processes it is waiting for

    def request_resource(self, resource_holders, initiator):
        """Initiates a probe if the resource request fails."""
        probe = [initiator, self.pid, resource_holders.pid]  # Probe message
        print(f"Process {self.pid} sending probe {probe} to {resource_holders.pid}")
        if resource_holders.receive_probe(probe):
    ...
    def receive_probe(self, probe):
        ...
        new_probe = [initiator, self.pid, self.waiting_for.pid]  # Modify probe
        print(f"Process {self.pid} forwarding probe {new_probe} to {self.waiting_for.pid}")
        if self.waiting_for.receive_probe(new_probe):
            return True
